        ...
        actions: [
          ...
          FilledButton(
            onPressed: () async {
              try {
                // 仮のファイル名でロードを試行
                final gameState = await GameSaveService.loadGame('default_save.json');
                if (gameState != null) {
                  // ゲーム状態を設定（initializeGameで初期化してから手動で状態を更新）
                  controller.initializeGame();
                  // 本来はcontrollerにloadGameStateメソッドが必要
                  if (context.mounted) {
                    Navigator.of(context).pop();
                    ScaffoldMessenger.of(context).showSnackBar(
                      const SnackBar(content: Text('ゲームを読み込みました')),
                    );
                  }
                } else {
                  if (context.mounted) {
                    Navigator.of(context).pop();
                    ScaffoldMessenger.of(context).showSnackBar(
                      const SnackBar(content: Text('保存データが見つかりません')),
                    );
                  }
                }
              } catch (e) {
