...
class AdvancedAIStrategy {
  ...
  AIThinkingResult _basicThinking(WaterMarginGameState gameState) {
    // 既存のAISystemを使用
    final aiSystem = AISystemFactory.createAI(factionId.name);
  ...
  SituationAnalysis _analyzeSituation(WaterMarginGameState gameState) {
    final ownedProvinces = gameState.provinces.values.where((p) => gameState.factions[p.name] == factionId).toList();

    final totalTroops = ownedProvinces.fold(0, (sum, p) => sum + p.military.round());
    final totalEconomy = ownedProvinces.fold(0, (sum, p) => sum + p.commerce.round());
    final averageSecurity = ownedProvinces.isEmpty
        ? 0
        : (ownedProvinces.fold(0.0, (sum, p) => sum + p.security) / ownedProvinces.length * 100).round();
  ...
  ThreatLevel _calculateThreatLevel(WaterMarginGameState gameState, List<Province> ownedProvinces) {
    ...
    for (final province in ownedProvinces) {
      for (final neighborName in province.neighbors) {
        ...
        if (neighbor != null && gameState.factions[neighbor.name] != factionId) {
          totalEnemyPower += neighbor.military.round();
          borderingEnemies++;
        }
      }
    }
        ...
        if (neighbor != null && gameState.factions[neighbor.name] != factionId) {
          ...
          if (powerRatio > 1.2) {
            ...
            opportunities.add(ExpansionOpportunity(
              ...
              economicValue: economicValue.round(),
  ...
  List<AIAction> _planMilitaryActions(WaterMarginGameState gameState) {
    ...
    for (final province in ownedProvinces) {
      ...
      final isfront = province.neighbors.any((neighborName) {
        final neighbor = gameState.provinces[neighborName];
        return neighbor != null && gameState.factions[neighbor.name] != factionId;
      });
